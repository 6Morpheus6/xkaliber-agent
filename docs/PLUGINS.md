...
- **Manually:** drop a plugin folder into `<userData>/plugins/<id>/` and restart.
  On Windows, `<userData>` is `%APPDATA%/Xkaliber Agent`.
- **Try the example:** copy `examples/plugins/hello` into your plugins dir.
...
Hook payloads: `beforeToolCall`/`afterToolCall` → `{ toolName, args[, result] }`;
`onPlanApproved` → `{ planId, goal, steps }`; `onPlanDone` → `{ planId, status, goal }`;
`onMessageSend` → `{ text }`. A failing hook is logged and ignored (it can't wedge
the agent); only a `beforeToolCall` returning `{ block: true }` stops a tool.
