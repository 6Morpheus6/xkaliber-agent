...
function applySearchReplace(content, find, replace) {
    if (content.includes(find)) {
        const n = content.split(find).length - 1;
        if (n > 1) return { error: `Multiple exact matches (${n})`, matchCount: n };
    ...
    if (matches.length > 1) {
        return { error: `Multiple whitespace-tolerant matches (${matches.length}). Add more surrounding context to the find block.`, matchCount: matches.length };
    }
    if (matches.length === 1) {
        const { i, j } = matches[0];
    ...
    return { error: 'No match for search/replace' };
}
