...
const { applySearchReplace, applyPatchToFile } = require('./lib/editFormats.js');
...
class EditEngine {
    ...
    async apply(planId, filePath, find, replace, opts = {}) {
        if (opts.dryRun) {
            ...
            const preview = applySearchReplace(content, find, replace);
        ...
        const result = applySearchReplace(lfBody, lfFind, lfReplace);
        if (result.error) {
        ...
        return { success: true, path: absPath, note: result.note };
    }
