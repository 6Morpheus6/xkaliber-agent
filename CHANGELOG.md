...
### Fixed
- **API Payload Compliance**: Fixed a critical bug causing the agent to hallucinate (e.g., mentioning "loopholes") by strictly enforcing the OpenAI tool response schema. Tool responses now accurately include the `name` field, and tool arguments are correctly stringified, preventing the AI backend from dropping the context history.
- **Resource Guard Task Isolation**: Fixed a logic error where the agent would mistakenly retain the *very first* message of the entire session instead of the most recent task when clearing context to save VRAM. The system now correctly searches backwards through history to isolate active tasks.
- **Thought Tag Leakage**: Implemented a streaming regex filter to intercept and strip raw model reasoning tags (e.g., `<|channel>thought <channel|>`) generated by advanced tool-enabled models, preventing them from leaking into the UI.

## [38.2.0] - 2026-05-18

### Fixed
- **Ultra-Aggressive Loop Pruning**: Fixed remaining LM Studio context cutoffs by forcing the context window to reserve 45% of its total capacity purely for code generation during active loops. This is critical for generating large files (like full web games).
- **In-Flight Context Wipes**: When the agent is deep in an autonomous loop (Step 2+), the system now continuously discards intermediate tool outputs, keeping only the original task definition, the system prompt, the current plan, and the immediate previous tool response.
- **Context Cutoff Recovery**: If the model *does* hit the `max_tokens` wall during an exceptionally massive code generation, the agent no longer aborts the entire task. Instead, it triggers an emergency memory wipe and injects a "SYSTEM ALARM" telling the AI that its file write failed due to size limits, allowing it to recover and try again in smaller chunks.

## [38.1.0] - 2026-05-18

### Added
- **Task-Aware Context Guard**: Re-engineered the memory management system to be "task-aware". The agent now explicitly identifies and preserves the original user instruction and the formal `task_begin` plan, even during aggressive context pruning.
- **Automatic Task Isolation**: The "Resource Saver" mode now triggers automatically when high system resource pressure (RAM/RSS) is detected, ensuring the model always has enough context for the current step.

### Fixed
- **LM Studio Context Overflow**: Resolved the "Guard Rail Error" and "Rolling Window" issues in LM Studio by implementing a more intelligent pruning algorithm that prioritizes the task definition over intermediate tool outputs.
- **Task Instruction Loss**: Fixed a bug where aggressive pruning would sometimes remove the original user request, causing the agent to lose focus on the goal.

## [38.0.0] - 2026-05-18

### Fixed
- **Ollama Reliability**: Resolved the "infinite thinking" hang when using Ollama models, particularly during multi-step file system tasks. This was caused by the autonomous loop failing to return the tool results to the model due to a missing continuation trigger.
- **Ollama JSON Parsing**: Added robust error detection and improved stream parsing for Ollama payloads.
- **Tool-Calling Context**: Fixed an issue where the model could lose context after multiple tool-calling turns by refining the autonomous workflow system prompt.

### Changed
- **Resource Defaults**: "Resource Saver" mode is now toggled **OFF** by default to prioritize reasoning and context retention on standard hardware.
- **System Prompt v38**: Updated core directives to be more authoritative regarding tool usage for system and file tasks.
