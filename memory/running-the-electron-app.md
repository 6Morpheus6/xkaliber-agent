...
description: How to launch and drive the Xkaliber Electron app headlessly for verification (binary download + Playwright + auth bypass)
metadata:
  type: reference
---
...
- **Drive it with `playwright-core`** (`_electron.launch`, executablePath `node_modules/electron/dist/electron.exe`, args `['--no-sandbox', '--user-data-dir=<temp>', APP_DIR]`). Windows has a real display — no xvfb. Use `--user-data-dir=<temp>` to keep tests off the real `%APPDATA%\xkaliber-agent` profile. `playwright-core` was added to devDependencies for this.
